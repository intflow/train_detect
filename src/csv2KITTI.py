...
train_label = pd.read_csv('./retinanet_type_annotation.csv', sep=',', header=None)
val_label = pd.read_csv('./retinanet_type_annotation.csv', sep=',', header=None)
mapping_info = pd.read_csv('./retinanet_type_class_mapping.csv', sep=',', header=None)
...
for index, rows in train_label.iterrows():
    ...
    with open(os.path.join(label_folder_path, file_name+'.txt'), 'a') as txt_file:
        ...
        txt_file.write(" ".join(map(str, img_info)) + '\n')
        # txt_file.write('\n')
